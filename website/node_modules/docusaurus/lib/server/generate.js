...
mdToHtml[metadata.source]=metadata.localized_id;
}
...
rawContent.replace(new RegExp(key,'g'),mdToHtml[key]);
});

var docComp=React.createElement(DocsPost,{config:siteConfig,metadata:_metadata,language:language,content:rawContent,__source:{fileName:_jsxFileName,lineNumber:100}});
...
var str=renderToStaticMarkup(React.createElement(ReactComp,{language:'en',__source:{fileName:_jsxFileName,lineNumber:118}}));
writeFileAndCreateFolder(targetFile,str);
}else
if(file.match(/\.css$/)){
console.log(file);
...
var str=renderToStaticMarkup(React.createElement(ReactComp,{language:'en',__source:{fileName:_jsxFileName,lineNumber:154}}));
writeFileAndCreateFolder(targetFile,str);
