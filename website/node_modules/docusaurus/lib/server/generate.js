...
files.sort().reverse().forEach(file => {
  const filePath = path.basename(file).replace('-', '/').replace('-', '/').replace('-', '/'
  ...
  const metadata = Object.assign({ path: filePath, content: rawContent }, result.metadata);
  metadata.id = metadata.title;

  let language = 'en';
  const blogPostComp = React.createElement(
    BlogPostLayout,
    { metadata: metadata, language: language, config: siteConfig },
    rawContent
  );
...
for (let page = 0; page < Math.ceil(MetadataBlog.length / perPage); page++) {
  ...
  const blogPageComp = React.createElement(BlogPageLayout, { metadata: metadata, language: language, config: siteConfig });
