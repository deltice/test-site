...
InlineLexer.prototype.outputLink = function (cap, link) {
  if (cap[0][0] !== '!') {
    const shouldOpenInNewWindow = link.href.charAt(0) !== '/' && link.href.charAt(0) !== '#';
...
Parser.prototype.tok = function () {
  ...
  switch (this.token.type) {
    ...
    case 'table':
      {
        ...
        for (i = 0; i < this.token.header.length; i++) {
          ...
          row.push(React.DOM.th(this.token.align[i] ? { style: { textAlign: this.token.align[i] } } : null, heading));
        }
        ...
        for (i = 0; i < this.token.cells.length; i++) {
          ...
          for (j = 0; j < cells.length; j++) {
            row.push(React.DOM.td(this.token.align[j] ? { style: { textAlign: this.token.align[j] } } : null, this.inline.output(cells[j])));
          }
    ...
    case 'paragraph':
      {
        return this.options.paragraphFn ? this.options.paragraphFn.call(null, this.inline.output(this.token.text)) : React.DOM.p(null, this.inline.output(this.token.text));
      }
    case 'text':
      {
        return this.options.paragraphFn ? this.options.paragraphFn.call(null, this.parseText()) : React.DOM.p(null, this.parseText());
      }
  }
};
...
function marked(src, opt, callback) {
  ...
  try {
    ...
    if ((opt || marked.defaults).silent) {
      return [React.DOM.p(null, 'An error occurred:'), React.DOM.pre(null, e.message)];
    }
    throw e;
  }
}
